...
for path in glob.glob("generators/*"):
    ...
    generators[generator] = importlib.import_module(f".{generator}", f"generators.{generator}")
...
if project['jdata']["toolchain"] == "icestorm":
    ...
    if project['osc_clock']:
        project['pinlists']["main"] = (("sysclk_in", project['jdata']["clock"]["pin"], "INPUT"),)
    else:
...
if "error" in project['jdata']:
    project['pinlists']["error"] = (("ERROR_OUT", project['jdata']["error"]["pin"], "OUTPUT"),)
...
# file structure
project['OUTPUT_PATH'] = f"Output/{project['jdata']['name'].replace(' ', '_').replace('/', '_')}"
...
os.system(f"cp -a files/LinuxCNC/Components/* {project['LINUXCNC_PATH']}/Components/")
os.system(f"mkdir -p {project['LINUXCNC_PATH']}/ConfigSamples/")
os.system(f"cp -a files/LinuxCNC/ConfigSamples/* {project['LINUXCNC_PATH']}/ConfigSamples/")
