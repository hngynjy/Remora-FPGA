...
def generate(project):
    ...
    top_arguments = []
    for _pname, pins in project['pinlists'].items():
        for pin in pins:
    ...
    top_data.append("")
    open(f"{project['SOURCE_PATH']}/remorafpga.v", "w").write("\n".join(top_data))
    project['verilog_files'].append("remorafpga.v")
    ...
    if project['jdata']["toolchain"] == "icestorm" and project['jdata']["family"] == "ecp5":
        ...
        ldf_data.append(
            f'<BaliProject version="3.2" title="remorafpga" device="{project["jdata"]["type"]}" default_implementation="impl1">'
        )
        ...
        ldf_data.append("")
        open(f"{project['FIRMWARE_PATH']}/remorafpga.ldf", "w").write("\n".join(ldf_data))
